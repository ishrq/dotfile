...
function sync -d "Sync files between devices"
    ...
    if test $USER = "u0_a1692" #S8
        ...
        rsync -auvz ~/.local/share/newsboat/ m10:~/.local/share/newsboat/
        rsync -auvz --exclude=git ~/.config/ m10:~/.config/

    else if test $USER = "u0_a215" #M10
        set dest "laptop"

        rsync -auzP ~/storage/shared/Documents/ s8:~/storage/shared/Documents/
        rsync -auvz ~/.local/share/newsboat/ s8:~/.local/share/newsboat/
        rsync -auvz --exclude=git ~/.config/ s8:~/.config/
